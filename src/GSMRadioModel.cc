...
#include "GSMRadioModel.h"
#include "Modulation.h"
#include "FWMath.h"


Register_Class(GSMRadioModel);

GSMRadioModel::GSMRadioModel()
{
    modulation = NULL;
}
...
void GSMRadioModel::initializeFrom(cModule *radioModule)
{
...
double GSMRadioModel::calculateDuration(AirFrame *airframe)
{
    return (airframe->getBitLength()+headerLengthBits) / airframe->getBitrate();
}


bool GSMRadioModel::isReceivedCorrectly(AirFrame *airframe, const SnrList& receivedList)
{
    // calculate snirMin
...
bool GSMRadioModel::isPacketOK(double snirMin, int length, double bitrate)
{
...
double GSMRadioModel::dB2fraction(double dB)
{
    return pow(10.0, (dB / 10));
}

double GSMRadioModel::getRSSI()
{
    return snir;
